---
title: "Text Mining alt-J Song Lyrics"
...
Love tops the chart when it comes to alt-J song lyrics. But what does the overall sentiment breakdown of alt-J song lyrics look like?

```{r sentiments-frequency, message=FALSE}
...
From this image it's pretty clear that the sentiment of alt-J songs tends to be positive. But how does this pattern look across different albums?

```{r sentiments, message=FALSE}
# sentiments of alt-J's songs
altj_lyrics %>%
  inner_join(get_sentiments("bing")) %>%
...
This chart takes the previous graph and separates out by album. Here we can see that 'This Is All Yours' was overwhelmingly positive, while 'Relaxer' was still positive, but by a smaller margin. However, there are repetitions of words between the albums. Can we instead try to find words that are used more in one album and not the others?

```{r unique-words,message=FALSE}
...
This chart shows the frequency of words in alt-J lyrics by album, that are not used often in other albums. For the curious, a quelea is a bird.

```{r unique-sentiment, message=FALSE}
## with sentiment
...
Finally, we repeat the same sentiment analysis, except with the unique words.
